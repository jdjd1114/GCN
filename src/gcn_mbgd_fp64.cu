                                    ...
                                    double * output )
{
	...
	if ( tid < output_size && bid < filter_num )
	{
        ...
        int head = data_id * cube_len * perLayerSize;
		int length = cube_len * perLayerSize;
		...
		copy_data_to_shared(filters, tmp + cube_len * perLayerSize, tid, offset, head, length);
		__syncthreads();

		double mid = 0;
		for(int i = 0; i < filterSize; i++){
            mid = mid + tmp[i + cube_len * perLayerSize] * tmp[tid * perLayerSize * stride + i];
		}
		mid = mid + bias[bid];
...
double training(double * data, double * labels, int x, int y, int z){
	...
	for (int j = 0; j < max_iter; j++ ){
		...
		for(int i0=0; i0<batch_num; i0++)
		{
			...
			//update parameters
			update_bias<<< NEU_NUM2, batch_size, sizeof(double) * batch_size >>>( batch_size, 
                                                                                    NEU_NUM2,
                                                                                    learning_rate, 
                                                                                    out.deltaB.data_d, 
                                                                                    out.bias.data_d );
